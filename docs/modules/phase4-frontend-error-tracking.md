...
- **T1-0 (docs):** this file.
- **T1-1 (backend, HARD GATE after):** `apps/api/src/routes/client-errors.ts` (+ test), mounted in `app.ts` (NOT `index.ts` — the prompt said index.ts; the actual mount table is `app.ts:37–54` via `createApp()`). Supporting: `lib/sentry.ts` (+`scrubText` export), `middleware/auth.ts` (+`tryReadUserId` best-effort), `lib/rate-limit.ts` (+`createCustomRateLimiter`), contract-test rate-limit mock (+`createCustomRateLimiter` passthrough). Awaiting channel approval before T1-2.
- **T1-2/T1-3/T1-4:** after approval.
