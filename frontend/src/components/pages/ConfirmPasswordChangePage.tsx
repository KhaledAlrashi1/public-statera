...
export default function ConfirmPasswordChangePage() {
  ...
  const submit = async () => {
    if (!token) {
      setStatus("error")
      setMessage("This password change link is incomplete or has expired.")
      return
    }
    setStatus("loading")
    try {
      ...
      setStatus("error")
      setMessage(err instanceof Error ? err.message : "We couldn't change your password right now.")
    }
  }
