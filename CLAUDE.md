...
2. **Queued fix-forwards** (NOT addressed in this run; scheduled for next session): (a) ~~**Encryption key empty-string fallback**~~ — **RESOLVED** in "module-7 smoke — <commit-sha>" below. (b) **`mapfile` portability** — `tools/module-7-smoke.sh` uses `mapfile -t` (bash 4+ only); macOS ships bash 3.2; fix: replace with portable `while IFS= read -r` loop. (c) **`assert_status` body-on-failure ergonomics** — status mismatches exit without printing the response body; the body contains the actual Hono error message; fix: dump the body on mismatch to avoid needing to re-issue curl manually. (d) **P0 stack health race** — P0 health check is one-shot; running the smoke test immediately after `docker compose restart` can abort before the API binds its port; fix: poll with 30 s timeout.
...
**module-7 smoke — <commit-sha>** `phase-3: module-7 fix-forward — encryption key empty-string fallback`
