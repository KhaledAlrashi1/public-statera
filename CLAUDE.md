...
- 8b Server bootstrap (DONE: deploy/bootstrap.sh — 13-section idempotent script: apt packages, deploy user, Hetzner Volume format+mount with device-path pattern validation, Docker CE + Compose v2.35.1, vm.overcommit_memory+swappiness sysctl, app directory + git clone, fail2ban 24h bantime, UFW, GHCR login guidance, unattended-upgrades with explicit 20auto-upgrades APT::Periodic settings, chrony with 5-min stabilisation warning, SSH hardening, deploy-user SSH verification before root lockout with Hetzner Rescue recovery procedure; deploy/.env.prod.example — all vars documented with required/optional, generation commands, grouped by concern; docker-compose.prod.yml volumes block updated to Option A bind-mount via driver_opts; Dockerfile corepack comment expanded with "Cannot find matching keyid" symptom; rebuild-from-scratch test passed: node:22.11.0-alpine@sha256:b64c, pnpm clean install 386 packages, tini apk, chown, GIT_SHA=8b-test stamped correctly into image sha256:bb2ba40f)
- 8c Secrets management (DONE: sops v3.9.1 + age v1.2.0 installed in bootstrap §1; §14 creates ~/.config/sops/age/ dir for deploy user; .sops.yaml at repo root with two-recipient pattern (operator + server age public keys — placeholders until post-bootstrap runbook runs); secrets/.env.prod.sops.yaml absent in initial commit by design (Option A — file created post-bootstrap per deploy/8c-post-bootstrap.md); dotenv plaintext format, YAML encrypted envelope; deploy invocation: docker compose --env-file <(sops -d --output-type dotenv secrets/.env.prod.sops.yaml); CI never sees plaintext (server-only decryption); deploy/SECRETS.md covers editing workflow, key rotation, SESSION_SECRET/MYSQL_PASSWORD rotation, ENCRYPTION_KEY rotation deferred; .gitattributes sopsdiffer entry; .gitignore guards for *.dec, decrypted/, .env.prod.filled)
- 8d CI/CD (DONE: .github/workflows/deploy.yml — test→build-push→deploy, concurrency serialized, workflow_dispatch with sha input for rollbacks; all third-party actions SHA-pinned; deploy job uses raw ssh with SendEnv=DEPLOY_SHA + command= forced command on server; deploy/deploy.sh — git reset --hard $GIT_SHA (not origin/main), decrypt-once pattern, compose() helper, _health_check() via docker compose exec, _rollback() with docker pull prev SHA + health check + loud failure if both versions unhealthy, PREV_SHA from OCI revision label; deploy/DEPLOY.md — CI key setup, authorized_keys command= + no-* restrictions, from= impracticality documented (6549 GitHub Actions IP ranges), AcceptEnv DEPLOY_SHA in sshd, action SHA pin update procedure, migration atomicity note; bootstrap.sh §12 updated to include AcceptEnv DEPLOY_SHA idempotently; additive migration rule added to CLAUDE.md standing rules)
