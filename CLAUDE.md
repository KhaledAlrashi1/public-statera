...
- 8c Secrets management (code committed 468696c + runbook fixes 8917c6a + §14 finding 537deb0 + key population 059bf30; sops v3.9.1 + age v1.2.0 in bootstrap §1; §14 key dir; .sops.yaml two-recipient pattern (operator + server age public keys committed in 059bf30); deploy/SECRETS.md; deploy/8c-post-bootstrap.md; .gitattributes sopsdiffer; .gitignore guards (incl. secrets/smoke.sops.yaml); **OPERATIONAL as of 2026-05-16** — runbook §1–§4 executed, smoke test passed (server decrypted clean dotenv output, no metadata leakage); remaining operational steps deferred: §5 (real production secrets file creation) and §6 (pre-8d manual deploy chain validation, intentionally deferred to be exercised by 8d's deploy.sh in production))
...
2. **Queued fix-forwards** (NOT addressed in this run; scheduled for next session): (a) ~~**Encryption key empty-string fallback**~~ — **RESOLVED** in "module-7 smoke — e2aa1cd" below. (b) ~~**`mapfile` portability**~~ — **RESOLVED** in "module-7 smoke — &lt;commit-sha-bcd&gt;" below. (c) ~~**`assert_status` body-on-failure ergonomics**~~ — **RESOLVED** in "module-7 smoke — &lt;commit-sha-bcd&gt;" below. (d) ~~**P0 stack health race**~~ — **RESOLVED** in "module-7 smoke — &lt;commit-sha-bcd&gt;" below.
...
**module-7 smoke — &lt;commit-sha-bcd&gt;** `phase-3: module-7 fix-forward — smoke test ergonomics (mapfile, assert_status, P0 race)`
