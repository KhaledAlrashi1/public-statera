...
- 8c Secrets management (code committed 468696c; runbook fixes in phase-3: 8c commit — operator key filename operator.key→keys.txt, existence guard + chmod 600, §4 smoke test rewrite to in-place encrypt on matching path, §6 pre-8d note, SOPS_AGE_KEY_FILE section removed, secrets/smoke.sops.yaml added to .gitignore; **NOT YET OPERATIONAL on server** — .sops.yaml still has placeholder age public keys, no secrets/.env.prod.sops.yaml created, post-bootstrap runbook §1–§6 not yet executed; next operational step: run deploy/8c-post-bootstrap.md §1–§4 and report smoke test output)
