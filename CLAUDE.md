...
3. **Why fixed in deploy.sh, not the workflow**: deploy.sh can be invoked manually outside the workflow (`GIT_SHA=<sha> bash deploy.sh`); one fix covers both invocation paths. The workflow's `inputs.sha` deliberately remains a free-form string.
